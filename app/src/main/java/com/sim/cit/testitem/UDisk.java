...
public class UDisk extends TestActivity {
	...
	private void startReadFile() {
		...
		File file = new File("/storage/usbotg/test.txt");
		Uri uri = Uri.parse("/storage/usbotg/test.txt");
		...
		// if(size < 3){
		// Toast.makeText(getApplicationContext(), R.string.insertSD,
		...
		//// Uri uri = Uri.parse("/storage/usbotg/test.txt");
		// Intent intent = new Intent();
		//// intent.setClassName("com.sim.udisktest",
		// "com.sim.udisktest.UDiskTest");
		// intent.setClass(getApplicationContext(), UDiskTest.class);
		//// intent.setAction(android.content.Intent.ACTION_VIEW);
		//// intent.setData(Uri.fromFile(file));
		// startActivityForResult(intent,1);
	...
	private void startSpeaker() {
		...
		// }
		/*
		 * if (mp != null) { Log.i(TAG,"mp != null"); mp.setLooping(true);
		 * mp.start(); }
		 */
	}
	...
	private void getUDiskStatus() {
		/*
		 * final String UDiskPath = "/storage/udisk"; String status =
		 * mStorageManager.getVolumeState(UDiskPath); Log.i(TAG,
		 * "UDisk status: " + status); if
		 * (Environment.MEDIA_MOUNTED.equals(status)) { StatFs stat = new
		 * StatFs(UDiskPath); Log.i(TAG, "UDiskPath "+UDiskPath); long blockSize
		 * = stat.getBlockSize(); long totalBlocks = stat.getBlockCount();
		 * sdIsPass = true; tvUDisk.setText(getString(R.string.sd_card_space) +
		 * Formatter.formatFileSize(this, (totalBlocks * blockSize))); }else{
		 * sdIsPass = false; tvUDisk.setText("UDisk:\tFail");
