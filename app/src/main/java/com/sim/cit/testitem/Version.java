...
//                mRilHook = new QcRilHook(this);
        ...
        tv[5].setText(SystemProperties.get("ro.product.hardware.version", "hardware"));
//        tv[5].setText(getHW_version());    //硬件版本
        ...
        tv[20].setText(getString(R.string.tp_type) + ":\n" +getTPManufacturerName()+"\n" + getTPManufacturerVer());
        ...
        tv[24].setVisibility(View.GONE);
        //Modify for add front camera information by lizhaobo 20151120 start
        tv[25].setText(getString(R.string.battery_vendor) + ":   " + getBatteryManufacturer());
        setSensor();
        tv[26].setText(getString(R.string.motion_sensor_vendor) + ":   " + motionSensor.getVendor() + "  " + motionSensor.getName());
        tv[27].setText(getString(R.string.proximity_sensor_vendor) + ":   " + proximitySensor.getVendor() + "  " + proximitySensor.getName());
        tv[28].setText(getString(R.string.light_sensor_vendor) + ":   " + lightSensor.getVendor() + "  " + lightSensor.getName());
    }
...
//		tv[18] = (TextView) findViewById(R.id.t19);
        ...
        //Modify for add front camera information by lizhaobo 20151120 start
        tv[25] = (TextView) findViewById(R.id.t26);     //battery
        tv[26] = (TextView) findViewById(R.id.t27);      //motion sensor
        tv[27] = (TextView) findViewById(R.id.t28);      //proximity sensor
        tv[28] = (TextView) findViewById(R.id.t29);      //light sensor
...
//		mRilHook.dispose();
    ...
    private String getTPManufacturerName() {
        String procCurrentStr;
        try {
            BufferedReader reader = new BufferedReader(new FileReader("/sys/readboy/tp_name"), 256);
            try {
    ...
    private String getTPManufacturerVer() {
        String procCurrentStr;
        String tempStr;
        try {
            BufferedReader reader = new BufferedReader(new FileReader("/sys/readboy/tp_ver"), 256);
            try {
                procCurrentStr = reader.readLine();
                if((tempStr = reader.readLine())!=null){    //最多读到第二行
                    procCurrentStr = procCurrentStr +"   "+ tempStr;
                }
            } finally {
                reader.close();
            }
    ...
    private String getLCDManufacturer() {
        ...
        try {
            ...
            BufferedReader reader = new BufferedReader(
                    new FileReader("/sys/readboy/lcd_name"), 256);
    ...
    private String getCameraManufacturer() {
        ...
        try {
            BufferedReader reader = new BufferedReader(
                    new FileReader("/sys/readboy/camera_rear"), 256);
            try {
                procCurrentStr = reader.readLine();
            } finally {
                reader.close();
    ...
    // Modify for add front camera information by lizhaobo 20151120 startEnable ADB Integration active.
    private String getFrontCameraManufacturer() {
        String procCurrentStr;

        try {
            BufferedReader reader = new BufferedReader(
                    new FileReader("/sys/readboy/camera_front"), 256);
            try {
                procCurrentStr = reader.readLine();
            } finally {
                reader.close();
    ...
    private String getBatteryManufacturer() {
        String procCurrentStr;

        try {
            BufferedReader reader = new BufferedReader(
                    new FileReader("/sys/readboy/battery_vendor"), 256);
            try {
                procCurrentStr = reader.readLine();
            } finally {
                reader.close();
    ...
    private void setSensor() {
        ...
        Log.e(TAG, motionSensor + "   " + proximitySensor + "   " + lightSensor);
    }
