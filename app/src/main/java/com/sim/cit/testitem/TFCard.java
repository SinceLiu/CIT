...
//        mStorageManager = StorageManager.from(this);
        tvSD = (TextView)findViewById(R.id.txt_one);
        btnPass = super.btnPass;
//        Intent service=new Intent().setComponent(DEFAULT_CONTAINER_COMPONENT);;
//    	bindService(service, mDefContainerConn, Context.BIND_AUTO_CREATE);
        int sizeMoreThanZero = 0;
        List<String> pathList = DiskManager.initDevicePath(this);
        StringBuilder sb = new StringBuilder();
        for(String path:pathList){
            long size = DiskManager.getVacantSpaceSize(path);
            long totalSize = DiskManager.getTotalSpaceSize(path);
            if(totalSize == 0)continue;
            sizeMoreThanZero ++;
            sb.append(path);
            sb.append(":    ");
            sb.append(size/1024.0f/1024/1024);
            sb.append(" G\n");
            sb.append("\r\n");
        }
        tvSD.setText(sb.toString());
        btnPass.setEnabled(sizeMoreThanZero >= 2);
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if(intent.getAction().equals(Intent.ACTION_MEDIA_MOUNTED))
                {
                    ...
                    try {
                        Thread.sleep(500);
                        int sizeMoreThanZero = 0;
                        List<String> pathList = DiskManager.initDevicePath(TFCard.this);
                        StringBuilder sb = new StringBuilder();
                        for(String path:pathList){
                            long size = DiskManager.getVacantSpaceSize(path);
                            long totalSize = DiskManager.getTotalSpaceSize(path);
