...
//		}
	 ...
	 */
	@SuppressWarnings("deprecation")
	public static long getTotalSpaceSize(String path) {
		StatFs fileStats = new StatFs(path);
		fileStats.restat(path);
		long size = ((long) fileStats.getBlockCount()*(long) fileStats.getBlockSize());
		Log.i(TAG, "getTotalSpaceSize() size = " + size);
		return size;
	}
}
